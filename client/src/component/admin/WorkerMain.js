...
class CustomerMain extends Component{
  ...
  getData = (callback) => {
    reqwest({
      url: 'http://localhost:5000/admin/getworker',
      type: 'json',
      method: 'post',
  ...
  delete = (id) => {
 		confirm({
		    ...
		    onOk : () => {
		      console.log('ok')
		      fetch('http://localhost:5000/admin/deleteworker', {
		                  method: 'POST',
		                  headers: {
  ...
  addcustomer = () => {
  				console.log(this.state.userName)
  			      fetch('http://localhost:5000/admin/addworker', {
		                  method: 'POST',
		                  headers: {
		                    'Accept': 'application/json',
		                    'Content-Type': 'application/json'
    ...
    return(
       <div>
       	<div style = {{'text-align' : 'right'}}>
       		<Input value = {this.state.userName}style = {{width : '200px',margin : '10px'}}placeholder="worker name" onChange = {this.onChange}>
			</Input>
       		<Button 
		      		type="primary" 
		      		shape="circle" 
